...
def attentive_node_features(emotions, seq_lengths, umask, matchatt_layer, no_cuda):
    ...
    for t in emotions:
        att_em, _ = matchatt_layer(emotions, t, mask=umask)
...
def classify_node_features(emotions, seq_lengths, umask, matchatt_layer, linear_layer, nodal_attn, no_cuda):
    """
...
class GraphNetwork(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_norm, edge_type, seq_lengths, umask, nodal_attn):
        ...
        log_prob = classify_node_features(emotions, seq_lengths, umask, self.matchatt, self.linear, nodal_attn, self.no_cuda)
        return log_prob

class DialogueGCNModel(nn.Module):

    def __init__(self, D_m, D_e, graph_hidden_size, n_speakers, max_seq_len, window_past, window_future,
                 n_classes=7, dropout=0.5, nodal_attention=True, no_cuda=False):
    ...
    def forward(self, U, qmask, umask, seq_lengths):
        ...
        features, edge_index, edge_norm, edge_type, _ = batch_graphify(emotions, qmask, seq_lengths, self.window_past, self.window_future, self.edge_type_mapping, self.att_model, self.no_cuda)
...
class MultiDialogueGCN(nn.Module):
    ...
    def __init__(self, modal, D_A, D_V, D_L, D_m, D_e, D_h, n_speakers, max_seq_len, window_past, window_future,
                 n_classes=7, dropout=0.5, nodal_attention=True, no_cuda=False):
        ...
        if "a" in modal:
            self.linear_a = nn.Linear(D_A, D_m)
            self.dialog_a = DialogueGCNModel(D_m, D_e, D_h, n_speakers, max_seq_len, window_past, window_future, n_classes, dropout, nodal_attention, no_cuda)
        if "v" in modal:
            self.linear_v = nn.Linear(D_V, D_m)
            self.dialog_v = DialogueGCNModel(D_m, D_e, D_h, n_speakers, max_seq_len, window_past, window_future, n_classes, dropout, nodal_attention, no_cuda)
        if "l" in modal:
            self.linear_l = nn.Linear(D_L, D_m)
            self.dialog_l = DialogueGCNModel(D_m, D_e, D_h, n_speakers, max_seq_len, window_past, window_future, n_classes, dropout, nodal_attention, no_cuda)

        self.smax_fc = nn.Linear(D_h*len(modal), n_classes)

    
    def forward(self, U_L, qmask, umask, lengths, U_A, U_V):
        """
        U -> seq_len, batch, D_m
        qmask -> seq_len, batch, party
        """
        ...
        if "a" in self.modal:
            hidden_a = self.linear_a(U_A)
            hidden_a = self.dialog_a(hidden_a, qmask, umask, lengths)
            hiddens.append(hidden_a)

        if "v" in self.modal:
            hidden_v = self.linear_v(U_V)
            hidden_v = self.dialog_v(hidden_v, qmask, umask, lengths)
            hiddens.append(hidden_v)

        if "l" in self.modal:
            hidden_l = self.linear_l(U_L)
            hidden_l = self.dialog_l(hidden_l, qmask, umask, lengths)
            hiddens.append(hidden_l)

        hidden = torch.cat(hiddens, dim=-1)
        
        log_prob = self.smax_fc(hidden)
        return log_prob, hiddens