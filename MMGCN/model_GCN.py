...
class GCNII_lyc(nn.Module):
    def __init__(self, nfeat, nlayers, nhidden, nclass, dropout, lamda, alpha, variant, return_feature, use_residue, new_graph=False):
        ...
        self.new_graph = new_graph # false
    ...
    def forward(self, x, dia_len, topicLabel, adj=None): # adj is not None
        if adj is None:
            if self.new_graph:
        ...
        for i,con in enumerate(self.convs):
            ...
            layer_inner = F.dropout(layer_inner, self.dropout, training=self.training) # 4329*100
            if i<3:
                layer_inners.append(layer_inner)
        ...
        return layer_inner,layer_inners # 4329*300
